...
// ── Mark buttons ──────────────────────────────────────────────────────────────
// FIX: always passes selectedDate so past-date writes work correctly
...
export default function Attendance() {
  ...
  // ── People list for manager table ────────────────────────────────────────────
  // Hierarchy: SuperAdmin → all TLs + all Recruiters
  //            CompanyAdmin → company TLs + company Recruiters
  //            TeamLead → their team's Recruiters only (they mark themselves separately)
  const allPersons = useMemo(() => {
    ...
    if (isTeamLead) {
      // Team lead sees only their team's recruiters in the table
      // (team lead marks themselves in the hero card above)
  ...
  return (
    <div className="content">
      ...
      {isRecruiter && (
        <div style={{
          ...
          {!myRecruiterProfile ? (
            ...
            <>
              {/* FIX: pass selectedDate so writing goes to correct date */}
              <MarkButtons
      ...
      {(isSuperAdmin || isCompanyAdmin) && (
        <div style={{
          ...
          <span>
            ...
            {' '}in your scope.
            {' '}Team Leads mark their own; you can override below.
          </span>
        </div>
      )}
      {isTeamLead && allPersons.length > 0 && (
        <div style={{
          ...
          <span>
            ...
            {' '}You can mark attendance for your team's recruiters below.
            {' '}Your own attendance is tracked in the card above.
          </span>
        </div>
      )}
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  <th>Marked At</th>
                  {/* FIX: Action column visible for all managers */}
                ...
                {pag.slice.map(person => {
                  ...
                  return (
                    <tr key={person.personId}>
                      ...
                      {/* Company / Team — only for superAdmin & companyAdmin */}
                      {(isSuperAdmin || isCompanyAdmin) && (
                      ...
                      {/* FIX: Action — managers can mark anyone; pass selectedDate */}
                      <td>
                        <MarkButtons
      ...
      {/* ── Recruiter: personal attendance history ───────────────────────────────── */}
